...
def add_buzzer_params(parser):
    ...
    parser.add_argument('--features', nargs='+', help='Features to feed into Buzzer', type=str,  default=["Category", "Length"])  
...
def load_buzzer(flags, load=False):
    ...
    for ff in flags.features:
        ...
        if ff == "Disambiguator":
            from features import DisambiguatorFeature
            feature = DisambiguatorFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)
        ...
        if ff == "Subcategory":
            from features import SubcategoryFeature
            feature = SubcategoryFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)
